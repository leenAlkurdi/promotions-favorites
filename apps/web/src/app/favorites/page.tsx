...
export default function FavoritesPage() {
  ...
  const tabs: { key: TabKey; label: string; count: number }[] = [
    {
      key: "active",
      label: t("favorites.tabs.active") || "Active",
      count: active.length,
    },
    {
      key: "expired",
      label: t("favorites.tabs.expired") || "Expired",
      ...
      <header className="flex flex-col gap-4">
        ...
        <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-white p-5 shadow hover:shadow-lg transition-shadow">
            ...
            <div className="flex flex-col">
              <p className="text-sm text-text-secondary">
                {t("favorites.aggregates.totalCount", {
                  count: aggregates.totalFavorites,
                })}
              </p>
              <p className="mt-1 text-2xl font-bold text-text-primary">
                {aggregates.totalFavorites}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-white p-5 shadow hover:shadow-lg transition-shadow">
            ...
            <div className="flex flex-col">
              <p className="text-sm text-text-secondary">
                {t("favorites.aggregates.totalRewards", {
                  amount: totalRewardsDisplay,
                })}
              </p>
              <p className="mt-1 text-2xl font-bold text-text-primary">
                {totalRewardsDisplay}
        ...
        <div className="flex flex-wrap items-center gap-3">
          {tabs.map((tabItem) => (
            ...
            >
              ...
              <span className="rounded-full bg-black/5 px-2 py-1 text-xs text-textSecondary">
                {tabItem.count}
              </span>
            </button>
          ))}
