...
export default function Card({
  ...
  const showExpiryBadge =
    typeof item.daysUntilExpiry === "number" && item.daysUntilExpiry <= 7;
  const formattedExpiry = item.expiresAt
    ? new Date(item.expiresAt).toLocaleDateString()
    : undefined;
  const daysLeft =
    typeof item.daysUntilExpiry === "number"
      ? Math.max(item.daysUntilExpiry, 0)
      : undefined;
        ...
        {showExpiryBadge && typeof item.daysUntilExpiry === "number" && (
          <div className="absolute left-3 top-3 bg-red-500 text-white text-xs px-2 py-1 rounded-full flex items-center gap-1">
            ...
            {item.daysUntilExpiry < 0
              ...
              : t("promotions.card.expiresIn", { days: item.daysUntilExpiry })}
          </div>
        )}
      </div>
        ...
        <div className="mt-auto flex items-center justify-between text-xs text-textSecondary">
          ...
          {daysLeft !== undefined && (
            <span className="font-medium text-gray-700">
              {t("promotions.card.expiresIn", { days: daysLeft })}
            </span>
          )}
        </div>
